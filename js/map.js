		...
		ajax_request.onreadystatechange = function()
		{
			if(ajax_request.readyState == 4 && ajax_request.status == 200)
			{
				...
				if(response.length > 0)
				{
					...
					{
						html += '<form action="add_prisoner.php" method="post" class="list-group-item list-group-item-action">' + '<input type="submit" name="prisoner_add" value="' + response[count].name + ' ' + response[count].surname + ', ' + response[count].prisoner_id + '">' + '<input type="hidden"  name="prisoner_add_id" value="' + response[count].prisoner_id + '">' + `<input type="hidden"  name="prisoner_add_cell_number" value="${localStorage.getItem('cell')}">` + '</form>';
					}
				}
				else
				{
