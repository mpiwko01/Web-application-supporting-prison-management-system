...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data['eventType']) && isset($data['date']) && isset($data['end']) && isset($data['visitor']) && isset($data['prisoner'])){
        ...
        //Wyliczenie ID spotkania
        $rowcount = mysqli_num_rows(mysqli_query($dbconn,"SELECT * FROM calendar_events")); //Pobranie liczby spotkań w tabeli
        $eventId = 0;
        if($rowcount == 0) $eventId = 1; //Jeśli w tabeli nie ma żadnego spotkania, przypisywane jest ID = 1
        else{
            for ($i=0; $i<$rowcount; $i++){ //Sprawdzenie czy istnieje jakieś niewykorzystane ID przed największym ID w tabeli
                ...
                if ($number['val'] == 0){ //Jeśli istnieje niewykorzystane ID to jest ono przypisywane dodawanemu spotkaniu
                    $eventId = $test;
                    break;
                }
            }
        ...
        if ($result1){
            ...
            //Sprawdzenie czy termin spotkania mieści się w okresie pobytu w więzieniu danego więźnia
            if(strtotime($row['to_date']) > strtotime($date)){ 
            ...
            } else echo json_encode(["status" => false, "msg" => "Więzień kończy swój wyrok: " . $row['to_date'] . ". Spotkania mogą odbyć się tylko przed tym dniem."]);
        }
    } else {
